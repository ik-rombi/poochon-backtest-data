...
def market_payload(slug: str) -> dict:
    return {
        "slug": slug,
...
def mock_client(payloads: dict[str, dict]) -> httpx.Client:
...
def test_discover_series_markets_returns_both_outcomes(monkeypatch) -> None:
    monkeypatch.setattr(
        ...
        lambda series, start_date, end_date: ["btc-updown-5m-1", "btc-updown-5m-2"],
    )
    client = mock_client({"btc-updown-5m-1": market_payload("btc-updown-5m-1")})
